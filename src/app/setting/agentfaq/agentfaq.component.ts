...
export class AgentfaqComponent implements OnInit {
  ...
  constructor(
    ...
    private AgentFeeServiceService: AgentFeeServiceService,
  ...
  search(pageurl = '') {
    this.spinner.show();
    const data = {
      category_type: this.searchForm.value.category_type,
      category_id: this.searchForm.value.category_id,
      faq_search: this.searchForm.value.faq_search,
    ...
    if (pageurl != '') {
      ...
      this.AgentFeeServiceService.getAllData(data).subscribe((res) => {
        this.agentFeeSlabs = res.data.data.data;
        this.pagination = res.data.data;
        this.all = res.data;
        this.spinner.hide();
  ...
  addFeeSlab() {
    ...
    if (id == null) {
      this.AgentFeeServiceService.create(data).subscribe((resp) => {
        if (resp.status == 1) {
      ...
      this.AgentFeeServiceService.update(id, data).subscribe((resp) => {
        if (resp.status == 1) {
          this.notificationService.addToast({
            title: Constants.SuccessTitle,
            msg: resp.message,
  ...
  editAgentFee(event: Event, id: any) {
    ...
    this.form = this.fb.group({
      ...
      price_from: [
        this.agentFeeSlabRecord.price_from,
        Validators.compose([Validators.required]),
      ],
      price_to: [
        this.agentFeeSlabRecord.price_to,
        Validators.compose([Validators.required]),
      ],
      max_comission: [
        this.agentFeeSlabRecord.max_comission,
        Validators.compose([Validators.required]),
      ],
    });
