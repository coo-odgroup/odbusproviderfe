...
export class MastersettingComponent implements OnInit {
  ...
  constructor(
    ...
    private busOperartorService: BusOperatorService,
  ...
  getAll(url: any = '') {
    ...
    this.settingsService.DataTable(url, data).subscribe((res) => {
  ...
  public pickedfav(event: any) {
    ...
    this.imageSizeFlag = true;
    if (event.target.files[0].size > max_size) {
      this.favError = 'Maximum size allowed is ' + max_size / 1024 + 'Kb';
      this.settingForm.value.imagePath = '';
      this.favURL = '';
      this.imageSizeFlag = false;
      return false;
  ...
  LoadAllService() {
    this.busOperartorService.readAll().subscribe((record) => {
  ...
  addSettings() {
    ...
    if (this.role_id != 1) {
      this.settingForm.controls.user_id.setValue(
    ...
    fd.append(
      'payment_gateway_charges',
      this.settingForm.value.payment_gateway_charges,
    );
    ...
    fd.append(
      'operator_home_content',
      this.settingForm.value.operator_home_content,
    );
    fd.append(
      'google_verification_code',
      this.settingForm.value.google_verification_code,
    );
    fd.append(
      'bing_verification_code',
      this.settingForm.value.bing_verification_code,
    );
    fd.append(
      'pintrest_verification_code',
      this.settingForm.value.pintrest_verification_code,
    );
    ...
    fd.append(
      'organization_schema',
      this.settingForm.value.organization_schema
    );
    ...
    if (id != null) {
      ...
      this.settingsService.update(fd).subscribe((resp) => {
        if (resp.status == 1) {
      ...
      this.settingsService.create(fd).subscribe((resp) => {
        if (resp.status == 1) {
          this.notificationService.addToast({
            title: 'Success',
            msg: resp.message,
  ...
  editSettings(id) {
    ...
    this.settingForm = this.fb.group({
      ...
      pintrest_verification_code: [
        this.settingRecord.pintrest_verification_code,
      ],
  ...
  deleteRecord() {
    ...
    this.settingsService.delete(delitem).subscribe((resp) => {
      if (resp.status == 1) {
        this.notificationService.addToast({
          title: 'Success',
          msg: resp.message,
  ...
  changeStatus(event: Event, stsitem: any) {
    this.spinner.show();
    this.settingsService.chngsts(stsitem).subscribe((resp) => {
      if (resp.status == 1) {
        this.notificationService.addToast({
          title: 'Success',
          msg: resp.message,
  ...
  removePopup(id) {
    ...
    this.settingsService.removePopup(id).subscribe((resp) => {
      if (resp.status == 1) {
        this.notificationService.addToast({
          title: 'Success',
          msg: resp.message,
