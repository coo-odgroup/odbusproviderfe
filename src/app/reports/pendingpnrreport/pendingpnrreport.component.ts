...
@Component({
  selector: 'app-pendingpnrreport',
  templateUrl: './pendingpnrreport.component.html',
  styleUrls: ['./pendingpnrreport.component.scss']
})
export class PendingpnrreportComponent implements OnInit {
  ...
  search(pageurl="")
  {
    ...
    if(pageurl!="")
    {
      this.rs.pendingpnrpaginationReport(pageurl,data).subscribe(
        res => {
    ...
    {
      this.rs.pendingpnrReport(data).subscribe(
        res => {
          this.completedata= res.data;
          // console.log( this.completedata);
          this.spinner.hide();
