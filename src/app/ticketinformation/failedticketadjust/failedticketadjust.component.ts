...
import { FailedticketadjustService } from '../../services/failedticketadjust.service';
...
@Component({
  selector: 'app-failedticketadjust',
  templateUrl: './failedticketadjust.component.html',
  styleUrls: ['./failedticketadjust.component.scss']
})
export class FailedticketadjustComponent implements OnInit {
  ...
  constructor(
    ...
    private rs: FailedticketadjustService,
  ...
  search(pageurl = "") {
    ...
    if (pageurl != "") {
      this.rs.failedticketadjustpaginationReport(pageurl, data).subscribe(
        res => {
      ...
      this.rs.failedticketadjustReport(data).subscribe(
        res => {
          this.completedata = res.data;
  ...
  active_status(id:any){
    const data = {
      id: id
    };
    this.rs.failedticketadjust(data).subscribe(
      res => {        
        if (res.status == 1) {
